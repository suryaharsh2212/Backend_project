...
app.use(cors({origin: process.env.CORS_ORIGIN})) // this middleware is a cross origin resource sharing which allows resouce sharing with the url we want 
// it has a origin :which specify where to talk to share resouce.
app.use(express.json({limit: "20kb"})) // this middleware is accepting and json data with a 20kb size
